...
import { Bookmark, ChevronLeft, ChevronLeftCircle, ChevronRight } from "lucide-react";
...
const Categories = () => {
  ...
  return (
    <div className="relative">
      <ChevronLeft className="absolute top-[15%] -left-6 cursor-pointer" size={27} onClick={scrollLeft} />
      <div className="flex items-center overflow-x-auto gap-5 hide_scroll scroll-smooth" ref={ref}>
        {categories.map((category, id) => (
          <div key={id} className="px-2 py-1 border flex-shrink-0 rounded-md bg-white">
            <p>{category}</p>
          </div>
        ))}
      </div>
      <ChevronRight className="absolute top-[15%] -right-6 cursor-pointer" size={27} onClick={scrollRight} />
    </div>
  );
};
